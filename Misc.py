...
class Cell:
    ...
    def wf_r(self, sq = True):
        ...
        x, y = np.meshgrid(X, Y)
        u = sol(x, self._height, self._AB, self._E) * \
            np.sin((np.pi/self._a) * y) 
...
class Simulation:
    ...
    def energy_scan(self):
        ...
        FuncAnimation(self._fig, self.scan_up, E, interval = 20, \
                      init_func = self.scan_init, blit = True, \
                      repeat = False)
        pl.show()
    
    
    def sup_init(self):
        X, Y, Z1 = snap(self._P, self._E1, mode = 'superposition')
        XY, Z1 = np.c_[X, Y], np.array(Z1)
        Z2 = np.array(snap(self._P, self._E2, \
                      mode = 'superposition')[2])
        Zt_sq = (Z1 + Z2) * np.conj(Z1 + Z2)
        
        self._scat.set_offsets(XY)
        self._scat.set_array(np.real(Zt_sq))
    ...
    def superposition(self, E1, E2):
        ...
        t = np.linspace(0, 10, num = 75)
        FuncAnimation(self._fig, self.sup_up, t, interval = 20,\
                      init_func = self.sup_init, \
...
def snap(A, E, mode = 'energy_scan', show = False):
    mode_valid = ['energy_scan', 'superposition', 'snapshot']
    if mode not in mode_valid:
        raise Exception("mode only take these as arguments: %r."\
                        % mode_valid)
    ...
    for i in range(row):
        for j in range(col):
            ...
            if mode != 'superposition':
                x, y, z = A.get_cell((i, j)).wf_r()
                z = np.real(z)
            else:
                x, y, z = A.get_cell((i, j)).wf_r(sq = False)
    ...
    if show and mode == 'snapshot': 
        fig, ax = pl.subplots()
        ax.scatter(X, Y, s = 3, c = Z)
        pl.show()
